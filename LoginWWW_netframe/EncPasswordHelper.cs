...
{
    public static class EncPasswordHelper
    {
        ...
        private static byte[] EncryptWithAesGcm(byte[] plainText, byte[] key, byte[] iv, byte[] tag, byte[] associatedData)
        {
            GcmBlockCipher cipher = new GcmBlockCipher(new AesEngine());
            AeadParameters parameters = new AeadParameters(new KeyParameter(key), tag.Length * 8, iv, associatedData);
